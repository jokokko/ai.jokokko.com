...
<div class="byline">Joona-Pekka Kokko · <a href="https://jokokko.com">jokokko.com</a> · <a href="https://github.com/jokokko">github.com/jokokko</a> · <a href="production-ai-engineering.pdf">PDF</a> · v1.0.1 · Last reviewed: 2026-05-14</div>
...
- **(2) Use prompt caching.** Stable prefixes can be much cheaper and faster, but the discount is provider- and model-specific. Put system prompt, tool schemas, and large reused docs at the front; verify cache hits in telemetry.
...
- **(4) Structured outputs via native APIs, not "please return JSON".** JSON-schema response formats, strict tool schemas, or constrained decoding. Validate with Pydantic/Zod and repair on failure.
...
Classical services are built around deterministic code paths and explicit state. LLM-backed services add a stochastic remote model. The contrast:
...
| Latency | Often milliseconds to low seconds; bounded by dependencies | Hundreds of ms to 60s+ depending on input, output, tools, and reasoning |
...
**Prompt caching is the highest-ROI optimization most teams skip.** Anthropic gives you explicit cache breakpoints (`cache_control`); OpenAI auto-caches stable prefixes on supported models. Cached input tokens can be much cheaper and faster, but read/write pricing and eligibility vary by provider. Order matters: stable content has to come before per-request inputs. Put system prompt, tool schemas, examples, and reused documents at the front, then check cached-token telemetry.
...
- **√dₖ** scales the dot products. Without it, in high dimensions the dot products grow large, softmax saturates (one weight near 1, the rest near 0), gradients collapse, the model can't learn nuanced multi-token attention. Square-root is right because dot products of independent unit-variance vectors have variance proportional to dₖ.
...
- **T → 0:** greedy decoding. Most repeatable. Use for extraction, classification, structured outputs, code where exact form matters.
...
**Decompose hard tasks.** For non-reasoning models, asking for a brief plan, checks, or intermediate artifacts can help on multi-step problems. For reasoning models, this is built in — ask for the explanation the user needs, not hidden chain-of-thought.
...
MCP is an open standard for connecting models to tools and data without per-integration glue code. It was introduced by Anthropic and is now governed through the Agentic AI Foundation under the Linux Foundation. The protocol uses JSON-RPC 2.0 over stdio or Streamable HTTP; SSE exists as legacy compatibility.
...
- **Streamable HTTP** — networked. Required for remote services. SSE is legacy compatibility. Auth is your problem — typically OAuth 2.1, sometimes bearer tokens.
...
- **Native structured output APIs** (OpenAI JSON-schema response formats or strict tool schemas, Anthropic tool-use schemas, Gemini response schemas) constrain decoding toward valid JSON matching your schema. Use these instead of post-hoc parsing.
- **Constrained / grammar-based decoding** (Outlines, llama.cpp grammars, vLLM guided decoding) enforces schema or regex during token sampling on open models. Stronger guarantee — the model literally can't emit invalid output — at some quality cost if the schema is over-specified.
...
- **Schema design matters.** Deeply nested or ambiguous schemas degrade quality. Prefer flat, well-named fields with clear `description` strings — the model sees those and treats them as inline docs. Provider "JSON Schema" support is usually a constrained subset, not a portable full JSON Schema runtime.
...
- **Keep the toolset small per agent.** Tool selection accuracy tends to degrade as the tool list grows. Beyond that, route between specialists with smaller toolsets, or retrieve the relevant tool subset per query.
...
- **Audit logs.** Every tool call with arguments, results, caller identity, confirmation state, and model-visible rationale or summary when available. Required for incident investigation and post-hoc behavioral analysis.
...
- **GGUF + llama.cpp quantizations (Q4_K_M, Q5_K_M)** — mixed precision per layer. Default for CPU / Apple Silicon / heterogeneous inference.
...
- Full trace: input messages, system prompt version, model + params, tool calls with args and results, retrieved context IDs, validation errors, final output, and provider-exposed reasoning summaries or reasoning-token counts where available. Do not depend on or store hidden chain-of-thought.
